...
export function ArticleCard({ article, matchScore }: { article: any, matchScore?: number }) {
    ...
    return (
        <Card
            onClick={handleClick}
            className="rounded-none border-zinc-200 bg-white hover:border-[#ED1C24] transition-all cursor-pointer group shadow-none relative"
        >
            <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4 border-b border-[#ED1C24] pb-2">
                    <div className="text-[10px] font-black text-[#ED1C24] uppercase tracking-[0.2em]">
                ...
                <h4 className="font-serif text-xl font-bold leading-tight group-hover:text-[#ED1C24] mb-3 transition-colors">
                    {article.title}
                </h4>
